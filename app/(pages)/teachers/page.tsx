...
export default function TeachersPage() {
      ...
      [
        ...new Map(
          teachers
            ...
            .map((t) => [
              t.universityFacultyId,
              { id: t.universityFacultyId!, name: t.universityFacultyShortName ?? '' },
            ])
        ).values(),
      ],
    [teachers]
  ...
  // Derive unique chairs from loaded data
        ...
        ...new Map(
          teachers
            ...
            .map((t) => [
              ...
              {
                id: t.universityFacultyChairId!,
                name: t.universityFacultyChairShortName ?? '',
                facultyId: t.universityFacultyId!,
              },
            ])
        ).values(),
      ],
  ...
  const filtered = useMemo(() => {
    ...
    return teachers.filter((t) => {
      ...
      const matchesFaculty =
        filters.facultyId == null || t.universityFacultyId === filters.facultyId

      const matchesChair =
        filters.chairId == null || t.universityFacultyChairId === filters.chairId

      const matchesStatus =
        filters.isActive === null || t.isActive === filters.isActive

      return matchesSearch && matchesFaculty && matchesChair && matchesStatus
    })
  }, [teachers, filters])

  const handleRowClick = (teacher: TeacherDto) => {
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            {isLoading
              ? 'Завантаження...'
              : `${filtered.length} з ${teachers.length} викладачів`}
          </p>
        </div>
        <SyncButton />
      </div>
