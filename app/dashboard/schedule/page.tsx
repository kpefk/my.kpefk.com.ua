...
export default function SchedulePage() {
  ...
  const todayWeekday = todayDow === 0 ? 7 : todayDow; // 1=Mon…6=Sat, 7=Sun
      ...
      <div className="flex gap-1 bg-muted rounded-2xl p-1 mb-6 overflow-x-auto">
        {DAYS_SHORT.map((label, i) => {
          ...
          return (
            ...
            >
              <span className={cn("font-semibold", isToday && !active && "text-kpefk")}>
                {label}
              </span>
              ...
              {hasLessons && (
                <span className={cn(
                  ...
                  active ? "bg-kpefk" : "bg-muted-foreground/40"
                )} />
              )}
            </button>
          );
      ...
      {lessons.length === 0 ? (
        ...
        <div className="space-y-3">
          {lessons.map((lesson) => (
            <div
              key={lesson.id}
              className="rounded-2xl border border-border bg-card p-4 sm:p-5 flex gap-4"
            >
              {/* Lesson number + time column */}
              <div className="shrink-0 flex flex-col items-center gap-1 min-w-[40px]">
                <span className="w-7 h-7 rounded-full bg-kpefk-light text-kpefk text-xs font-bold flex items-center justify-center">
                  {lesson.lessonNumber}
                </span>
              ...
              <div className="flex-1 min-w-0 pb-1">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <Clock className="w-3.5 h-3.5 text-kpefk shrink-0" />
                    {lesson.startTime} – {lesson.endTime}
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <DoorOpen className="w-3.5 h-3.5 text-kpefk shrink-0" />
                    Аудиторія {lesson.room}
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                    <User className="w-3.5 h-3.5 text-kpefk shrink-0" />
                    {lesson.teacher}
                  </div>
                </div>
              </div>
