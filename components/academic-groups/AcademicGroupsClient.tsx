...
export function AcademicGroupsClient() {
  ...
  const curatorOptions = useMemo<CuratorOption[]>(
    () => teachers.map((t) => ({ id: t.id, fullName: getFullName(t), positionName: t.positionName })),
    [teachers],
  )
  ...
  const uniqueEducationForms = useMemo(
    () =>
      [...new Set(groups.map((g) => g.educationFormName).filter((f): f is string => f !== null))].sort(
        (a, b) => a.localeCompare(b, 'uk'),
      ),
    [groups],
  )
  ...
  const filtered = useMemo(() => {
    ...
    return groups.filter((g) => {
      ...
      const matchesFaculty = filters.facultyName === null || g.facultyName === filters.facultyName
      const matchesForm = filters.educationFormName === null || g.educationFormName === filters.educationFormName
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            {isLoading ? 'Завантаження...' : `${filtered.length} з ${groups.length} груп`}
          </p>
        </div>
        <GroupSyncButton />
      </div>
