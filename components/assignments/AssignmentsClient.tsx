...
function deadlineText(deadline: Date, status: AssignmentStatus): { text: string; urgent: boolean } {
  if (status === 'graded' || status === 'submitted') {
    return { text: deadline.toLocaleDateString('uk-UA', { day: 'numeric', month: 'long' }), urgent: false }
  }
  const now = new Date()
  ...
  return { text: `${deadline.toLocaleDateString('uk-UA', { day: 'numeric', month: 'long' })} · ${diff} дн.`, urgent: false }
}
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
        {FILTERS.map(({ key, label }) => (
          ...
          >
            {label}
            {counts[key] > 0 && (
              <span className={cn('ml-1.5 text-xs', filter === key ? 'opacity-75' : 'opacity-60')}>
                {counts[key]}
              </span>
            )}
          </button>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((assignment) => {
            ...
            return (
              <div key={assignment.id} className="rounded-2xl border border-border bg-card p-4 sm:p-5">
                <div className="flex items-start justify-between gap-3 mb-2">
                  ...
                  <div className={cn('shrink-0 flex items-center gap-1 text-xs font-medium px-2.5 py-1 rounded-full', cfg.bg, cfg.color)}>
                    ...
                    {cfg.label}
                    {assignment.grade !== undefined && <span className="ml-0.5 font-bold">{assignment.grade}</span>}
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  ...
                  <div className={cn('flex items-center gap-1', dl.urgent && 'text-destructive font-medium')}>
                    {dl.urgent ? <Clock className="w-3 h-3" /> : <CalendarClock className="w-3 h-3" />}
                    {dl.text}
                  </div>
                </div>
              </div>
