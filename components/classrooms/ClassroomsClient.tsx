...
export function ClassroomsClient() {
  ...
  const filtered = useMemo(() => {
    ...
    return classrooms.filter((c) => {
      ...
      const matchesTeacher =
        filters.hasTeacher === null || (filters.hasTeacher ? c.teacher !== null : c.teacher === null)
      const matchesPhotos =
        filters.hasPhotos === null || (filters.hasPhotos ? c.photos.length > 0 : c.photos.length === 0)
  ...
  return (
    <div className="flex flex-col gap-6 p-4 sm:p-6">
      ...
      <ClassroomFormDialog open={createOpen} onClose={() => setCreateOpen(false)} />
    </div>
  )
}
