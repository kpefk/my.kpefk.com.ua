...
import { StudentActionsMenu } from '@/features/students/components/student-actions-menu'
...
export function StudentsClient() {
		...
		() => [
			...new Map(
				students
					...
					.map(s => [
						s.educationFormId,
						{ id: s.educationFormId!, name: s.educationFormName ?? '' }
					])
			).values()
		],
		[students]
	...
	const filtered = useMemo(() => {
		...
		return students.filter(s => {
			...
			const matchesForm =
				filters.educationFormId == null || s.educationFormId === filters.educationFormId
			const matchesStatus =
				filters.isActive === null || isStudentActive(s) === filters.isActive
			...
			<div className='flex flex-wrap items-center justify-between gap-4'>
				<div>
					...
					<p className='text-muted-foreground mt-0.5 text-sm'>
						{isLoading
							? 'Завантаження...'
							: `${filtered.length} з ${students.length} студентів`}
					</p>
				</div>
				<div className='flex shrink-0 items-center gap-2'>
					...
					/>
					<StudentActionsMenu
						studentIds={filtered.map(student => student.id)}
						canProvision={userRole === 'ADMINISTRATOR'}
					/>
				</div>
			</div>
